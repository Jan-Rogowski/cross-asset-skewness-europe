...
RET_PATH = DATA_PROCESSED_DIR / "fx_monthly_returns.csv"
SKEW_PATH = DATA_PROCESSED_DIR / "fx_skewness_12m.csv"
PORTFOLIO_PATH = DATA_PROCESSED_DIR / "portfolio_fx_skewness_ls.csv"
...
def load_data() -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    print("Waluty:", common_cols)
    print("Miesiące:", len(common_idx))
...
def build_skewness_portfolios(
    ...
    for date in rets.index:
        ...
        long_idx = sig.index[:k]
        short_idx = sig.index[-k:]
...
def main():
    ...
    print(f"Zapisano portfel FX do: {PORTFOLIO_PATH}")
